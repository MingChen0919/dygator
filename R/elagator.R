...
#' \code{init_elagator} initialize a elastic galay tool repository
#'
#' @export
init_elagator = function() {
  ...
  elagator_template_dir = system.file('extdata', package = 'elagator')
  from_files = list.files(elagator_template_dir, pattern = 'elastic_*', full.names = TRUE)
  to_files = paste0('elagator/', unlist(lapply(strsplit(from_files, '/'), tail, 1)))
  dir.create('elagator')
  if (all(file.copy(from_files, to_files, overwrite = TRUE))) {
    cat('elagator is successfully initialized.\n')
  } else {
    cat('initialization failed.\n')
  }
}

#' \code{reinit_elagator} overwrite the existing elastic galaxy tool repository with a new one.
#'
#' @export
reinit_elagator = function() {
  if (dir.exists('elagator')) {
...
#' @export
add_requirements = function(packages, versions) {
  if (!dir.exists('elagator')) {
    stop('elagator does not exist. Please run init_elagator() to initialize a tool first.')
  }

  macros_xml_content = read_xml('elagator/elastic_tool_wrappers_macros.xml')
  ...
  write_xml(macros_xml_content, file = 'elagator/elastic_tool_wrappers_macros.xml')
}
...
remove_requirements = function(requirements) {
  if (!file.exists('elagator/elastic_tool_wrappers_macros.xml')) {
    stop('elagator/elastic_tool_wrappers_macros.xml does not exist. Please make sure you have a elagator directory
...
#' \code{add_tools} add tools to 'elagator/elastic_tool_wrappers_macros.xml'.
#'
...
it_tool = function(id='tool_id', name='tool name', version='1.0.0') {
  if (!file.exists('elagator/elastic_tool.xml')) {
    stop('elagator/elastic_tool.xml does not exist. Please make sure you have a elagator directory
         in your working directory')
  }

  xml_content = read_xml('elagator/elastic_tool.xml')
